...
### 6.3 已获取数据集（2026-08-05 补充）

通过 git clone 从 GitHub 成功获取：

| 数据集 | 规模 | 语言 | 转换状态 |
|---|---|---|---|
| **SmileChat** | 55,120条中文对话 | 中文 | ✅ convert_smilechat.py |
| **ESCoT** | 1,707条 | 英文 | ✅ convert_escot.py |

**SmileChat 领域分布**（自动标注）：
DS-INT 21,011 > DS-FAM 17,670 > DS-LRN 15,567 > DS-REL 14,158 > DS-WRK 11,908 > DS-SLP 3,082 > DS-FIN 1,319 > DS-CAR 797 > DS-PRC 366 > DS-MIG 146

> 注：SmileChat领域标签为规则自动推断，需后续抽样精标注。策略标签为默认值。
...
**恢复方案**：尝试GitHub直连（VPN环境下）、ModelScope镜像、或作者提供的国内网盘。
...
| V1.2 | 2026-08-05 | 补充SmileChat(55K)+ESCoT(1.7K)，更新待获取清单 |
